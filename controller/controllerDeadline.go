...
func CekDeadline() {
	...
	for rows.Next() {
		if err := rows.Scan(&listEmail.Etherium, &listEmail.Title, &listEmail.Username, &listEmail.Email, &listEmail.Date, &listEmail.MarketId); err != nil {
			log.Println(err)
			return
		} else {
